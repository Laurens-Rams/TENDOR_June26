...
namespace Immersal.XR
{
    ...
    {
        // TENDOR PATCH: Screen.orientation is a main-thread-only API, but AdjustForScreenOrientation is
        // called from a background localization thread (Localizer.CollectLocalizationResults) which throws
        // "GetScreenOrientation can only be called from the main thread" on Unity 2023.2+ and kills the
        // Immersal session. We read Screen.orientation from this cache, which is refreshed every frame on
        // the main thread by BodyTracking.Spatial.ImmersalDelayedInitializer.
        public static ScreenOrientation CachedScreenOrientation = ScreenOrientation.Portrait;
        ...
        {
            // TENDOR PATCH: use the main-thread-cached orientation instead of Screen.orientation so this
            // method is safe to call from the background localization thread.
            ScreenOrientation so = overrideOrientation ?? CachedScreenOrientation;
