...
{
    /// <summary>
    /// Math + Inference Engine helpers for the Blaze family of models. Ported (with generics intact) from the
    /// official Unity sentis-samples BlazeDetectionSample (Pose). Kept self-contained so the BlazePose runner
    /// can reuse the exact affine-sampling pipeline the sample validates against.
    /// </summary>
    public static class BlazeUtils
    {
        ...
        public static float[,] LoadAnchors(string csv, int numAnchors)
        {
            ...
            // Tolerate CRLF and blank trailing lines so a stray newline doesn't break parsing.
            var anchorLines = csv.Split(new[] { '\r', '\n' }, System.StringSplitOptions.RemoveEmptyEntries);

            if (anchorLines.Length < numAnchors)
            {
                throw new System.FormatException(
                    $"Anchors CSV has {anchorLines.Length} rows but the detector needs {numAnchors}. " +
                    "This is the wrong anchors file - use the 2254-row anchors.csv from the Unity sentis-samples " +
                    "BlazeDetectionSample/Pose/Assets/Data/anchors.csv (matches pose_detection.onnx, 224x224).");
            }
